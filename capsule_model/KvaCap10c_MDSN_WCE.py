...
class WeightedTripletMarginLoss(losses.TripletMarginLoss):
    def __init__(self, margin=1.5, swap=False, smooth_loss=False, triplets_per_anchor="all", weight=None, normalize_weight=True, **kwargs):
        super().__init__(margin=margin, swap=swap, smooth_loss=smooth_loss, triplets_per_anchor=triplets_per_anchor, **kwargs)
        self.weight = weight
        if self.weight is not None and normalize_weight:
            self.weight = self.weight / self.weight.max()

    def compute_loss(self, embeddings, labels, indices_tuple, ref_emb, ref_labels):
        loss_dict = super().compute_loss(embeddings, labels, indices_tuple, ref_emb, ref_labels)

        loss = loss_dict['loss']['losses']

        if loss is None:
            raise ValueError(f"The loss_dict does not contain 'losses'. Actual keys: {list(loss_dict.keys())}")

        if self.weight is not None:
            anchor_idx, _, _ = indices_tuple
            anchor_labels = labels[anchor_idx]
            anchor_weights = self.weight[anchor_labels]

            # 直接使用 .cuda() 将 anchor_weights 移动到 GPU
            anchor_weights = anchor_weights.cuda()

            weighted_loss = loss * anchor_weights
...
def train(_train_loader, _valid_loader, _test_loader, _model, _criterion, _optimizer, _scheduler, _args):
    ...
    best_acc_path=""
    best_f1_path=""
    miner = miners.TripletMarginMiner(margin=1.5, type_of_triplets="semihard", distance=SNRDistance())
    c_weight = torch.FloatTensor([47.9330, 90.6332, 82.0356, 261.0692, 53.4923, 70.1182, 1.2089, 754.7273, 14.2842, 48.6066])
    loss_func = WeightedTripletMarginLoss(margin=1.5, triplets_per_anchor="semihard", distance=SNRDistance(), weight=c_weight)
    for epoch in range(_args.epochs):
        ...
        for i, (inputs,targets) in enumerate(tqdm(_train_loader)):
            ...
            w2 = 1-w1
            loss1 = loss_func(embeddings,targets,semihard_pairs) * 300
            loss2 = _criterion(outputs,targets)
            # print(f"loss1_triplet={loss1}, loss2_CE={loss2}")
...
def main():
    ...
    logger.info("MDSN-WCE: train information on {} dataset\n".format(args.dataset))
    ...
    # class_weight=torch.FloatTensor([47.9330, 90.6332, 82.0356, 261.0692, 53.4923, 70.1182, 1.2089, 754.7273, 14.2842, 48.6066])
    weight=torch.FloatTensor([47.9330, 90.6332, 82.0356, 261.0692, 53.4923, 70.1182, 1.2089, 754.7273, 14.2842, 48.6066])
    ...
    criterion = get_criterion(weight,args)
    optimizer = get_optimizer(model,args)
    scheduler = get_scheduler(optimizer,_T_max=32)
