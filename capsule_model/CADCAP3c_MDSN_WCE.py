...
def train(_train_loader, _valid_loader, _test_loader, _model, _criterion, _optimizer, _scheduler, _args):
    ...
    loss_func = losses.TripletMarginLoss(margin=1.5,triplets_per_anchor="semihard", distance=SNRDistance())
...
def evaluate(_test_loader,_model,_criterion,_args):
    ...
    logger.info("Test: Loss: {:.4f}, Accuracy: {:.4f} %, Time: {:.4f}s".format(avg_test_loss,
                                                                               avg_test_acc * 100,
                                                                               test_end - test_start))
    torch.save(_model, "./model/dataset{}{}classes{}_testacc{:.4f}_f1score{:.4f}_Triplet_model{}_bs{}_seed{}_epochs{}_optim{}_lr{}_wd{}.pt".format(_args.dataset,
                                                                                                                                           _args.num_classes,
                                                                                                                                           _args.datadir[-3:],
                                                                                                                                           avg_test_acc,
                                                                                                                                           F1_score,
...
def main():
    ...
    logger.info("RE: train information on RE dataset\n")
    ...
    class_weight=torch.FloatTensor([3, 3, 3])
